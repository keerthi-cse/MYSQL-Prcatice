...
SELECT 
    ...
    WEEKDAY(date),
    DAYNAME(date),
    DATEDIFF(DATE(utc_timestamp()),date) AS total_days, -- diff between curr date(utc) to date value in the table
    ADDDATE(date,2),
    SUBDATE(date,2),
    CAST('2025-01-01' AS datetime) -- coverting string to date
FROM 
	dim_date;
...
SELECT 
	*,
    ...
    CONCAT_WS(' ',first_name,last_name,country),  -- WithSeperator
